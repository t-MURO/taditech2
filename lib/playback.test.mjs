...
test("clamps player volume and uses a safe fallback", () => {
  assert.equal(clampPlaybackVolume(-0.2), 0);
  assert.equal(clampPlaybackVolume(0.43), 0.43);
  assert.equal(clampPlaybackVolume(1.4), 1);
  assert.equal(clampPlaybackVolume(Number.NaN), 0.72);
});
